...
# 3) Models
# -----------------------------
def build_lstm_fused(window, out_dim, units=96, dropout=0.10):
...
def build_lstm_decomposed(window, out_dim, units=96, dropout=0.10):
    """
...
if __name__ == "__main__":
    ...
    # Model + train
    if USE_DECOMPOSED:
        model = build_lstm_decomposed(window=window, out_dim=2*horizon, units=UNITS, dropout=DROPOUT)
    else:
        model = build_lstm_fused(window=window, out_dim=2*horizon, units=UNITS, dropout=DROPOUT)
