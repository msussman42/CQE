...
\begin{itemize}
\item Callison and Chancellor\cite{callison2022hybrid} ``Hybrid quantum-classical algorithms in the noisy intermediate-scale quantum era and beyond''
...
\item 
Cerezo et al\cite{cerezo2021variational} ``Variational quantum algorithms''
\item
...
\item
Lotshaw et al\cite{doi:10.1098/rsta.2021.0414} ``Simulations of frustrated Ising Hamiltonians using quantum approximate optimization''
...
\item Kim et al\cite{kim2023evidence}, ``Evidence for the utility of quantum computing before fault tolerance''
\item Liu et al\cite{liu2024natural}, ``A Natural Primal-Dual Hybrid Gradient Method for Adversarial Neural Network Training on Solving Partial Differential Equations''
\item Saha et al\cite{saha2025high}, ``High-fidelity remote entanglement of trapped atoms mediated by time-bin photons''
\item De Luo et al\cite{luo2025quantumsimulationbubblenucleation},
''Quantum simulation of bubble nucleation across a quantum phase transition''
%de.luo@duke.edu
%fsurace@caltech.edu Federica Maria Surace
%c.monroe@duke.edu Christopher Monroe
\item Chen et al\cite{chen2025non}, 
``Non-invasive mid-circuit measurement and reset on atomic qubits''
\item Yu et al\cite{yu2025situ},
``In-situ mid-circuit qubit measurement and reset in a single-species trapped-ion quantum computing system''
\item Tennant et al\cite{PRXQuantum.3.030307}, 
``Low-Frequency Correlated Charge-Noise Measurements Across Multiple Energy Transitions in a Tantalum Transmon''
\item Fei et al\cite{fei2023binary},
...
\item Viamontes et al\cite{viamontes2004high}, ``High-performance QuIDD-based simulation of quantum circuits''
\item Zhang et al\cite{zhang2024quantum}, ``Quantum circuit simulation with fast tensor decision diagram''
\item Kluber\cite{kluber2023trotterization}, ``Trotterization in quantum theory''
\item Zoufal et al\cite{zoufal2019quantum}, ``Quantum generative adversarial networks for learning and loading random distributions''
\item Drozdov\cite{drozdov1998improved}, ``Improved Feynman’s path integral method with a large time step: Formalism and applications''
\item Dong\cite{SpancerDongthesis}, ``The self-consistent field energy landscape and its application to describing electron correlation''
\item Jamet et al\cite{jamet2025anderson}, ``Anderson impurity solver integrating tensor network methods with quantum computing''
\item Gleissner et al\cite{gleissner2024restricted}, ``Restricted global optimization for QAOA''
\item McClean et al\cite{mcclean2018barren}, ``Barren plateaus in quantum neural network training landscapes''
\item Nghiem and Tzu-Chieh\cite{nghiem2023quantum}, ``Quantum algorithm for estimating largest eigenvalues''
\item Jiang et al\cite{jiang2025review}, ``A Review on Zeroing Neural Networks''
\item Cadi et al\cite{cadi2024folded}, ``Folded spectrum vqe: A quantum computing method for the calculation of molecular excited states''
\item Blote and Deng\cite{blote2002cluster}, ``Cluster Monte Carlo simulation of the transverse Ising model''
\item Zheng et al\cite{zheng2004numerical}, ``Numerical solutions of the Schr{\"o}dinger equation for the ground lithium by the finite element method''
\item Bravyi and Kitaev\cite{bravyi2002fermionic}, ``Fermionic quantum computation.''
\item Pritchard et al, ``New basis set exchange: An open, up-to-date resource for the molecular sciences community''
\item Tilly et al\cite{tilly2022variational}, ``The variational quantum eigensolver: a review of methods and best practices''
\item McArdle et al.\cite{mcardle2020quantum}, ``Quantum Computational Chemistry''
\item Jiang et al.\cite{jiang2025walking}, ``Walking through Hilbert space with quantum computers''
\item O'Malley et al.\cite{PhysRevX.6.031007}, ``Scalable Quantum Simulation of Molecular Energies''
\item Qing and Xie\cite{qing2023usevqecalculateground}, ``Use VQE to calculate the ground energy of hydrogen molecules on IBM Quantum''
\item Quiroz et al\cite{quiroz2025quantifying}, ``Quantifying the impact of precision errors on quantum approximate optimization algorithms''
\item Weaving et al\cite{weaving2025contextual}, ``Contextual subspace variational quantum eigensolver calculation of the dissociation curve of molecular nitrogen on a superconducting quantum computer''
\item Ryu et al\cite{VQSVD}, ``A Variational Quantum Algorithm for Ordered SVD''
\item Sim et al\cite{sim2019expressibility}, ``Expressibility and entangling capability of parameterized quantum circuits for hybrid quantum-classical algorithms''
\item Shirakawa et al\cite{PhysRevResearch.6.043008}, ``Automatic quantum circuit encoding of a given arbitrary quantum state''
\item Krinner et al\cite{krinner2022realizing}, ``Realizing repeated quantum error correction in a distance-three surface code''
\end{itemize}
...
%https://www.editorialmanager.com/jcomp/
\par\noindent

%Publication date (estimate): 30 October 2025.
Editorial acceptance deadline: 30 May 2026
\par\noindent
Another Invitation template:
\par\noindent
subject: invitation to submit article(s) to the Journal of Computational Physics special issue "Numerical Methods for Engineering Quantum Computers''
\par\noindent
body: Dear Drs. Xinyu Fei, Lucas Brady, and Jeffrey Larson,
Your research in general, and your recent article in particular, "Binary Control Pulse Optimization for Quantum Systems," caught my attention.  I am an associate editor for the Journal of Computational Physics, and together with guest editors Drs.  L. Lin (Berkeley),   A. Petersson (LLNL), S. Guenther (LLNL), S. Jin (Shanghai Jiao Tong), and P. Lotshaw (ORNL) we are forming a special issue "Numerical Methods for Engineering Quantum Computers." Please consider submitting article(s) to our special issue.  The scope is:
...
Submission website:
\verb=https://www.editorialmanager.com/jcomp/=
Select the article type of ``VSI: Numerical Methods for Engineering Quantum Computers''
Submission portal closes: 30 November, 2025.
...
\par\noindent
%%Gracia Lyu g.lyu@elsevier.com
%%Selvakumar Rajendran s.rejendran@elsevier.com
%%Maggie Yang m.yang@elsevier.com
%%Dongbin Xiu xiu.16@osu.edu
%%VSI: Numerical Methods for Engineering Quantum Computers
%%Search above key phase in google for more sources.
